...
public class Assassin : Enemy, IMoveable, IHealth
{
    ...
    private async UniTask Attack(Animal target, List<Block> range)
    {
        ...
        AnimalHealth health = target.GetComponent<AnimalHealth>();
        int diff = health.Maxhealth - health.health;
        int add = (int)(diff * percent);

        target.ModifyHealth(-(Info.Damage + add));
